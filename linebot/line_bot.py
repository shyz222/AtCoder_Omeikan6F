...
def scraping():
    ...
    while True:
        try:
            ...
            parsed = soup.find_all("div",attrs={"id": "contest-table-upcoming"})
            # extract time and contest info by using regex
            data = str(parsed[0])
            search_result_time = re.findall(r"<time class=\"fixtime fixtime-full\".+>",data)
            ...
            for t,c in zip(search_result_time,search_result_content):
                ...
                if today == date_time[:10]:
                    line_message = "本日（{0}） {1}があります".format(date_time,content)
                    print(line_message)
            ...
            if len(content_L_old) < len(content_L):
                ...
                for _num in range(new_content_num,0):
                    line_message = "新しいコンテストが追加されました\n {0} \n {1}".format(content_L[_num][0],content_L[_num][1])
                    print(line_message)
                    push_massage(line_message)
...
def push_massage(line_message):
    LINE_BOT_API.push_message(TARGET_LINE_ID, TextSendMessage(text=line_message))
